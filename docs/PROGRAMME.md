...
The three things that would settle it are cheap and none of them is being done: event-based stream sampling in the days after spreading, copper and zinc with veterinary residues in stream and marine sediment rather than in mussels alone, and the rendering and slurry-tank volumes that would say how much material is in the system in the first place. Until then the correct sentence is **not** *the faeces are the problem*. It is: **the account has one channel, the source has two, and only the first has ever been weighed.**
