...
**The direct evidence that it is incomplete is its own history.** <span class="claim" data-claim="C-HY-GROWTH">The first version had [71](SOURCES.md#F-bee553f05a) entries and was written to be thorough. It reached [127](SOURCES.md#F-595b2862fe) within a single afternoon, and every addition came from an analogy raised in passing — soil sickness, desertification, sandy deserts, compost going anaerobic, turfgrass thatch, replant disease. None of those came from searching the marine literature. A list that grows by three quarters in one conversation is not a list anyone should call complete</span><sup class="claim-mark">[†](CLAIMS.md#C-HY-GROWTH "What this claim rests on")</sup>, and there is no reason to think the next conversation would add fewer.
...
Nobody has published where each Danish area sits in that window, and <span class="claim" data-claim="C-HY-ONE-THRESHOLD">iltsvind is defined nationally by oxygen concentration alone, which assumes the tolerable level is the same everywhere</span><sup class="claim-mark">[†](CLAIMS.md#C-HY-ONE-THRESHOLD "What this claim rests on")</sup>.
...
This is not hypothetical at either end. Inside a dense bloom, <span class="chem" data-chem="CO2" title="carbon dioxide">CO₂</span> really is drawn down far enough to push pH up sharply — which is the carbon entry in the element sweep below, and the reason a bloom can poison water by consuming carbon rather than by producing anything. It corrects within a day, as soon as respiration resumes. Bottom-water oxygen depletion does not correct at all; it compounds.
...
And the first step carries the measurement priming has otherwise lacked. `AtzA`, the atrazine chlorohydrolase, and `TriA`, the melamine deaminase, are [98](SOURCES.md#F-84d87fba5e)% identical proteins — a handful of amino acids apart — and they catalyse *different reactions*, one stripping a chlorine and the other stripping an amine. **A handful of substitutions is the distance between one novel capability and another.** That is what "the distance from existing machinery to the required function" means when it is put in units, and it is why the supply of mutations is not the limiting term: nine is nothing, if you are already standing in the right place.
...
|---|---|---|---|---|
| `C` | **Carbon** — <span class="chem" data-chem="CO2" title="carbon dioxide">CO₂</span> and bicarbonate; the vast DIC pool | Rarely limiting in bulk, but drawn down inside a dense bloom, where pH can rise steeply. | High pH shifts ammonium toward un-ionised ammonia, which is acutely toxic. A bloom therefore poisons the water by consuming carbon — a kill with no hypoxia in it at all. | established |
...
| `K` | **Potassium** — ~[400](SOURCES.md#F-540b0a2249) mg/kg in seawater | No. Never limiting. | None. | n/a |
| `Mg` | **Magnesium** — ~[1.3](SOURCES.md#F-2277f0280a) g/kg in seawater | No. | None. | n/a |
...
|---|---|---|
| `O1` | **Oxygen deficit** | Dissolved oxygen below [4](SOURCES.md#F-cf14a00b21) or [2](SOURCES.md#F-1ceed0aafd) mg/L, by depth, duration and extent. **An observable on route [M1](HYPOTHESES.md "Depletion of something essential"), not an outcome.** Nobody values a gas concentration; it earns its place only through what it causes, and it is neither necessary nor sufficient for any of T1-T5. |
...
| `O9` | **Bathing water failure** | Closures and quality downgrades. The one outcome Denmark measures densely, over a long period ([what the data holds](HYPOTHESIS_DATA.md#O9)). |
...
**Predicts.** Deficit scales with current-year N load, strongest where N-limitation is long and flushing is slow. A load reduction should show within a few years.

**Discriminated by.** The N-load coefficient after controlling for wind work, stratification and imported COD - not the R² of a bundle containing all of them.
...
**Predicts.** Spring chlorophyll tracks P, and the summer deficit tracks spring chlorophyll better than it tracks summer N.

**Discriminated by.** Spring (March-May) chlorophyll against P load, and the deficit against spring rather than summer production. DCE exclude spring from the indicator.
...
Selective catalytic reduction injects urea solution - AdBlue, [32.5](SOURCES.md#F-492e40b543)% urea - into diesel exhaust, where it decomposes to ammonia and reduces nitrogen oxides to harmless <span class="chem" data-chem="N2" title="dinitrogen gas">N₂</span> and water.

**The composition is worth stating plainly, because it makes the entry obvious.** `AUS 32` under `ISO 22241` is two ingredients: [32.5](SOURCES.md#F-492e40b543)% urea, [67.5](SOURCES.md#F-492e40b543)% demineralised water. Urea, `CO(NH2)2`, is [46.6](SOURCES.md#F-00f96001f6)% nitrogen by mass, so the fluid is [15.2](SOURCES.md#F-1c5433a022)% nitrogen — against [28](SOURCES.md#F-558b6e1b0f)–[32](SOURCES.md#F-558b6e1b0f)% for the liquid UAN fertilisers spread on fields. **AdBlue is a half-strength fertiliser solution distributed through the fuel network**, and urease, the enzyme that converts urea to ammonium, is present in essentially all soil and water and acts within hours. Anything reaching ground as liquid — spillage at filling points, tank rinsing, disposal — is fertiliser arriving by that route, with no combustion involved at all. That is a second pathway entirely separate from slip, and neither is measured.
...
**Predicts.** Step changes at plant upgrades; a distance-decay signal from the outfall.

**Discriminated by.** Deficit against per-plant discharge, exploiting upgrade dates as natural experiments.
...
**Predicts.** Signal enters at the boundaries and propagates inward with the circulation; correlates with Baltic inflow events, not with Danish load.

**Discriminated by.** Boundary nutrient concentration and transport against interior deficit.
...
**Predicts.** Summer DIP rising above the annual mean - which DCE already use as an iltsvind indicator, i.e. they measure the mechanism and treat it as a symptom.

**Discriminated by.** Benthic flux chambers, or the DIP seasonality ratio against the sediment organic pool.
...
**Discriminated by.** Sediment and fauna transects radiating from each licensed site.

**Needs.** Per-farm production and feed use by month; sediment stations near farms. [What the data holds →](HYPOTHESIS_DATA.md#A8)
...
**Needs.** Species-level phytoplankton counts; N-fixation rate measurements.
...
Rain overwhelms a combined system and raw sewage discharges directly: organics, fat, faecal solids, at [1](SOURCES.md#F-e8c64a3b0a) g <span class="chem" data-chem="O2" title="oxygen (dissolved, as a molecule)">O₂</span> demand per g COD.
...
**Needs.** Per-outfall overflow volume and duration per event - the single most valuable missing series. [What the data holds →](HYPOTHESIS_DATA.md#B1)
...
**Needs.** Event-resolved concentration and volume at the outfall, and basin sediment surveys. Annual totals exist and cannot test this; one published estimate bears directly on the settling basins. [What the data holds →](HYPOTHESIS_DATA.md#B2)
...
**Discriminated by.** Deficit against PE density, controlling for treatment stage.

**Needs.** Per-plant monthly COD/BOD discharge. [What the data holds →](HYPOTHESIS_DATA.md#B3)
...
**Predicts.** Scales with discharge and erosion, so it co-varies with N load and is systematically misattributed to it.

**Discriminated by.** POC flux measured alongside N flux at the same stations. Almost never done.
...
**Predicts.** Very high COD per unit volume; localised; tied to production schedules.

**Discriminated by.** Per-permit COD discharge against local deficit.
...
**Predicts.** Extreme local deficit in harbours; a plausible direct source of shore fat.

**Discriminated by.** Oxygen and organic content inside harbour basins, which are rarely monitored.
...
**Predicts.** Follows traffic density and lane geometry rather than any catchment; scrubber washwater is acidic and metal-bearing.

**Discriminated by.** Deficit and contaminant signature against AIS traffic density.
...
**Predicts.** Extreme, brief, local. Fish kills. Under-reported by construction.

**Discriminated by.** Reported pollution incidents against fish-kill records.

**Needs.** Environmental incident register with date and location.
...
**Predicts.** Deficit tracks the strength and persistence of the pycnocline, and is near zero in well-mixed water whatever the load.

**Discriminated by.** Potential energy anomaly from CTD profiles against deficit.
...
**Predicts.** Calm summers produce deficits; windy ones do not, at identical load.

**Discriminated by.** Cumulative wind work over the stratified season against deficit.

**Needs.** Hourly wind. Already held. [What the data holds →](HYPOTHESIS_DATA.md#C2)
...
**Predicts.** The same load produces very different outcomes at different flushing times, so any load coefficient that is not normalised by residence time is wrong.

**Discriminated by.** Deficit against load divided by flushing time, rather than against load.
...
**Predicts.** Episodic, large, and entirely exogenous to Danish policy.

**Discriminated by.** Deep salinity and oxygen against inflow event chronology.
...
**Predicts.** The physical effect of discharge is confounded with the nutrients in it, and the two have never been separated.

**Discriminated by.** Discharge volume as a predictor separate from discharge concentration.
...
Warmer water holds less oxygen and respires faster, with demand rising with Q10.

**Predicts.** Deficit rises with bottom temperature even at constant organic supply.

**Discriminated by.** Deficit against *bottom* temperature. Not a candidate variable in the statistical models, which carry surface temperature only.
...
**Predicts.** Deficit is a function of geometry, and is where it always was.

**Discriminated by.** Deficit against sill depth and basin volume below the pycnocline.
...
**Predicts.** Step changes at construction dates, localised downstream of the works. Testable as natural experiments with known dates.

**Discriminated by.** Before-and-after deficit at fixed stations either side of each work.
...
**Predicts.** Slow, monotone, and confounded with every other trend.

**Discriminated by.** Exchange volume reconstruction against deficit.

**Needs.** Tide gauge records. Available and long.
...
**Predicts.** Fauna loss follows effort spatially with no oxygen anomaly needed; turbidity and oxygen demand spike along tracks.

**Discriminated by.** Fauna and sediment redox against trawling effort at fine spatial resolution.

**Needs.** VMS/AIS-derived trawling effort. **Obtainable:** the ICES/HELCOM swept-area layers on figshare (`20310255`), not fetched. Design in [`hypodrafts/D1.md`](hypodrafts/D1.md). [What the data holds →](HYPOTHESIS_DATA.md#D1)
...
**Predicts.** Localised, dated, permitted - therefore highly testable.

**Discriminated by.** Before-and-after at fixed stations near dredging campaigns.
...
**Discriminated by.** Fauna and sediment chemistry at and downstream of dumping grounds against dumping volume and date.

**Needs.** Per-ground dumping volume, date and material chemistry. [What the data holds →](HYPOTHESIS_DATA.md#D3)
...
**Discriminated by.** Fauna inside against outside extraction areas, over time.

**Needs.** Per-area extracted volume by year. Permits are public; volumes less so. [What the data holds →](HYPOTHESIS_DATA.md#D4)
...
**Predicts.** Linear disturbance with known route and date.

**Discriminated by.** Fauna along versus away from routes, before and after.
...
**Predicts.** Follows anchorage polygons and vessel draught.

**Discriminated by.** Bed condition inside versus outside designated anchorages.
...
**Predicts.** Follows wave bed shear stress, and is the mechanism most likely to *deliver* fedtemøg to a shore rather than create it.

**Discriminated by.** Shore fouling reports against modelled bed shear stress.

**Needs.** Wave hindcast. Partially held: bed shear already modelled from the wind record. [What the data holds →](HYPOTHESIS_DATA.md#C2)
...
**Predicts.** **A seabed can become a desert without being poisoned or suffocated.** Once the skin is broken the bed mobilises, abrasion and burial kill the neighbours, and the damage propagates laterally - the crust-and-vehicle-track story exactly. The state then holds itself, because mobility prevents recolonisation and recolonisation is what would restabilise it.

**Discriminated by.** Critical erosion threshold and microphytobenthic biomass against disturbance history, rather than against nutrients.
...
**Predicts.** Removing the structure does not average the system; it collapses it to the barren phase, which is a lower-productivity *stable* state rather than an intermediate one. A survey reporting means across an area cannot see the difference between a mosaic and its barren half.
...
**Predicts.** Sediment composition shifts permanently with no change in supply, and the organic matter removed is deposited somewhere else, concentrating demand elsewhere.

**Discriminated by.** Grain size distribution through time at fixed stations.
...
**Predicts.** Erodibility is a biological property with a sign that can flip. Neither side of the balance appears in nutrient assessment, so a bed can change from stable to mobile with no measured change in anything.

**Discriminated by.** The ratio of bioturbating to biostabilising biomass against measured erodibility.

**Needs.** Species-level fauna with functional traits assigned. The fauna data exists in ODA; the trait assignment is a desk exercise.
...
Reduced sulphur from anoxic sediment consuming [2](SOURCES.md#F-5320d7e8e5) g <span class="chem" data-chem="O2" title="oxygen (dissolved, as a molecule)">O₂</span> per g S the moment it meets oxygenated water.
...
**Discriminated by.** Sediment sulphide pools and porewater against oxygen demand.
...
Ammonium oxidised to nitrate, consuming [4.57](SOURCES.md#F-8412cafaa5) g <span class="chem" data-chem="O2" title="oxygen (dissolved, as a molecule)">O₂</span> per g N with no biology of interest in between.
...
**Predicts.** Small per gram but large in total where sediment is iron-rich.

**Discriminated by.** Porewater Fe(II) flux against deficit.
...
Methane from anoxic sediment consuming [4](SOURCES.md#F-845bc5868c) g <span class="chem" data-chem="O2" title="oxygen (dissolved, as a molecule)">O₂</span> per g on its way up.

**Predicts.** Seep-associated, localised, and invisible to nutrient accounting.

**Discriminated by.** Methane flux surveys.
...
**Predicts.** Consumes oxygen without any organic matter or nutrient involved.

**Discriminated by.** COD measured on discharges as such.
...
**Predicts.** Fauna loss concentrated near marinas, harbours and lanes with no oxygen anomaly at all. TBT imposex is a documented Danish effect.

**Discriminated by.** Fauna and sediment biocide concentration together at the same stations.

**Needs.** Sediment biocide concentrations, at sea. [What the data holds →](HYPOTHESIS_DATA.md#E6)
...
**Predicts.** Scales with PE, not with agriculture; effects are sublethal and chronic.

**Discriminated by.** Effluent and receiving-water concentrations against fauna condition.
...
**Discriminated by.** Biota and sediment time series.
...
**Predicts.** Kills without hypoxia; worst in warm alkaline water near outfalls.

**Discriminated by.** Un-ionised ammonia calculated from ammonium, pH and temperature.
...
**Predicts.** Kills at the bed before oxygen reaches zero; produces the sensory signature that generates public complaints.
...
**Predicts.** Chlorophyll rises with no change in nutrient supply. Reversible by restoration, which makes it a testable intervention.

**Discriminated by.** Filter-feeder biomass against chlorophyll at fixed nutrient load.
...
**Predicts.** A positive feedback: hypoxia kills bioturbators, whose loss deepens hypoxia. Explains hysteresis without invoking anything else.

**Discriminated by.** Bioturbation potential index from fauna data against sediment oxygen penetration depth.
...
**Predicts.** Turbidity and resuspension rise together after vegetation loss, which then prevents recovery. DHI model this explicitly.

**Discriminated by.** Vegetation depth limit against turbidity, with the direction of causation tested by lag.
...
**Predicts.** Step changes at arrival dates, spreading spatially from an introduction point.

**Discriminated by.** Community composition before and after documented arrivals.
...
**Predicts.** Episodic; the carcass fall is a concentrated oxygen sink.

**Discriminated by.** Jellyfish abundance against subsequent deficit.
...
**Predicts.** Kills at chlorophyll levels that the indicator scores as acceptable, because the indicator counts biomass and not identity.

**Discriminated by.** Species-level phytoplankton composition against mortality events.
...
**Predicts.** High productivity and high biomass with low diversity - which the chlorophyll indicator cannot distinguish from health, and which is what fedtemøg is.

**Discriminated by.** Diversity and turnover rate alongside biomass.
...
**Predicts.** Sudden, species-specific, and not preceded by any oxygen anomaly.

**Discriminated by.** Mortality event records with species and date.
...
**Predicts.** Local, sudden, and a direct route to shore fouling.
...
**Predicts.** Monotone worsening independent of load, and it explains why load reduction has not produced the expected recovery.

**Discriminated by.** Deficit against bottom temperature, and the load coefficient estimated within temperature strata.
...
**Predicts.** Event concentration rises even as annual totals fall - so a falling annual load can coexist with rising peak impact.

**Discriminated by.** Flux delivered in the top decile of days, as a fraction, over time.
...
**Predicts.** Deficit worsens with no change in load or temperature.

**Discriminated by.** Trend in seasonal wind work over the record.
...
**Predicts.** Chronic, monotone, worst for shelled species.

**Discriminated by.** Carbonate chemistry against calcifier condition.
...
**Predicts.** Shifts in the phenology of the whole annual cycle.

**Discriminated by.** Ice extent against bloom timing.
...
**Predicts.** Slow and confounded, but affects the geometry every other hypothesis assumes.

**Discriminated by.** Exchange reconstruction.

**Needs.** Tide gauges. Long and available.
...
**Predicts.** Recovery requires a much larger reduction than the one that caused the collapse, and the response is not a function of the current year's load at all. Consistent with the observed failure of load reduction to restore eelgrass.

**Discriminated by.** Whether the load-response relation differs between the declining and recovering phases - a test nobody has published.
...
**Predicts.** Deficit correlates with cumulative past load, not current load, and lags by years to decades. DCE name this mechanism as the reason their own models drift, and then leave it out of the models.

**Discriminated by.** Cumulative load with a fitted decay against current load.
...
**Predicts.** The dose-response steepens over time at constant stressor.

**Discriminated by.** Whether the fitted coefficient changes across eras.
...
**Predicts.** A hump. Fitting a straight line through three simulated points, as the official method does, cannot recover it.

**Discriminated by.** Non-parametric response of production and diversity to load.

**Needs.** Load and response across a wide gradient - which the marine water bodies supply. [What the data holds →](HYPOTHESIS_DATA.md#H4)
...
Redfield C:N:P at [106](SOURCES.md#F-fa1bf80bf8):[16](SOURCES.md#F-fa1bf80bf8):1 and, for diatoms, Si:N at [15](SOURCES.md#F-2c8ffebf10):[16](SOURCES.md#F-2c8ffebf10) (Redfield-Brzezinski) are requirements, not averages. Skewing the ratios changes which organisms can complete their life cycle, independently of how much of anything there is.
...
**Discriminated by.** Community composition against N:P and Si:N, with absolute concentrations held fixed - which the marine water bodies make possible.

**Needs.** Simultaneous N, P and Si with species-level counts. [What the data holds →](HYPOTHESIS_DATA.md#H4)
...
Distinct from turbidity as a symptom: for a rooted plant, light at the bed is a resource with a hard requirement - a minimum share of surface irradiance for eelgrass - and below it the plant does not grow slowly, it dies.
...
The floor is the eelgrass requirement, a minimum share of surface irradiance. The ceiling is real too: photoinhibition and UV damage at the surface, which is why some species do worse in the clearest water.
...
**Discriminated by.** Response across the full irradiance range, not against a single threshold.
...
**Predicts.** Community composition shifts with no change in Kd's magnitude, because Kd is one number where the mechanism is a spectrum. A wholly separate driver, arriving from the same catchments and correlated with nutrient load, which makes it a confounder as well as a cause.

**Discriminated by.** Spectral attenuation, or at minimum CDOM absorbance, alongside Kd.
...
**Needs.** Wave and current modelling. Bed shear already computed here from the wind record. [What the data holds →](HYPOTHESIS_DATA.md#C2)
...
**Predicts.** Behavioural effects along cable routes, which are dated and mapped - so this is testable even though it is speculative.

**Discriminated by.** Distribution and behaviour against cable routes, before and after installation.
...
**Predicts.** **Kd is measured and its cause is assumed.** A Kd exceedance is attributed to phytoplankton because phytoplankton is what the framework is about, but the same number is produced by resuspended mineral sediment, by browning, by a dredging plume, or by detritus that grew somewhere else and drifted in. This is the residual-estimator problem in the light indicator: the quantity is real and measured, and the attribution behind it is not.

**Discriminated by.** Spectral attenuation partitioned into its components, against each candidate source separately.
...
**Predicts.** A nutrient effect on light that produces no change in Kd at all, because the shading happens after the light has passed through the water. So the nutrient-to-light pathway can operate with the light indicator reading normally - and grazers that would have cropped the epiphytes are the ones removed by every other route here.

**Discriminated by.** Epiphyte load per unit leaf area against nutrient status and grazer abundance.
...
**Predicts.** Turbidity with a known date, a known position and a known operator - so this is one of the few attenuation sources that is a natural experiment rather than a background condition.

**Discriminated by.** Turbidity and vegetation response near works, before and after, against controls at distance.
...
**Discriminated by.** Host condition indices measured alongside both the stressor and the pathogen, rather than any one of the three.

**Needs.** Carbohydrate reserves, tissue sulphide and pathogen load on the same plants. Done in research; not in monitoring.
...
**Predicts.** **A light-and-sulphide interaction that is neither a nutrient effect nor a water-column oxygen effect.** Both indicators can read acceptably while the plant dies, because the lethal condition is in the sediment and the trigger is at the surface. Explains dieback in water whose measured oxygen never fell.

**Discriminated by.** Sulphide in eelgrass tissue against light at the bed and sediment sulphide - the three together, which is the whole point.
...
**Predicts.** The proximate cause of death is a pathogen; the reason it succeeded is a stressor. Attribution to either alone is wrong, and a survey recording only the die-off sees neither.

**Discriminated by.** Pathogen prevalence alongside host stress indicators, rather than either alone.
...
**Discriminated by.** Transplant trials into failed sites, with and without sediment inoculation from a functioning bed.
...
Some soils suppress disease purely through their microbial community, and suppressiveness is transferable - mix a small share of a suppressive soil into a conducive one and it becomes suppressive. Anaerobiosis and fumigation destroy it.
...
**Discriminated by.** Whether sediment from a healthy bed confers resistance when mixed into a failed one - the standard transfer assay.
...
**Predicts.** Plants die in anoxic sediment for several reasons at once, only one of which is lack of oxygen. A model with an oxygen term and nothing else attributes all of it to oxygen.

**Discriminated by.** Porewater organic acids and reduced metals alongside sulphide.
...
**Predicts.** Hypoxia does not only weaken the host; it favours the pathogen. Two effects in the same direction, so the observed damage exceeds what an oxygen-tolerance curve predicts.
...
**Predicts.** A single national retention figure, or a coefficient fitted to national averages, describes nowhere. The regulatory pressure assigned to an individual farm is a soil model output wearing a water-policy label, for a quantity its own producers say cannot be measured directly.

**Discriminated by.** Measured leaching from paired sites on contrasting soils under the same management - which is the only thing that would validate the map.
...
**Predicts.** The land stores nutrient on a different timescale from the one policy operates on, in both directions - slow to fill and slow to empty. Symmetrical with the iron shuttle in the sediment, and for the same mineralogical reason.

**Discriminated by.** Desorption isotherms alongside the sorption ones, which are the half usually measured.
...
**Predicts.** A sediment can be P-rich and P-poor at once. Any budget built on totals mis-states both the stock and the flux, and the error is not small.

**Discriminated by.** Sequential fractionation of marine sediment phosphorus, the standard soil method applied to the bed.
...
The Leptosol lesson: a shallow soil over rock has almost no capacity to absorb a shock, because buffering is proportional to the volume of material doing the buffering.
...
**Discriminated by.** Response amplitude against water depth and sediment thickness, at matched load per unit area.
...
**Predicts.** The same application delivers more to the sea now than it did in 1990, so a coefficient fitted on the early record over-states present retention and under-states present delivery. Fitting a constant to a declining quantity also produces exactly the systematic drift DCE report in their own TN models.

**Discriminated by.** Retention estimated separately by era, rather than fitted once across the whole record.

**Needs.** The same catchment flux data, split by period. Requires no new measurement at all.
...
Decomposer microbes build biomass at a low C:N and respire much of the carbon they take in, so there is a threshold C:N: below it decay releases mineral nitrogen, above it decay consumes it. Straw, far above the threshold, starves the next crop. Fat has no nitrogen at all.
...
**Discriminated by.** Dissolved inorganic nitrogen drawdown following organic-carbon inputs of known C:N, against inputs of the same carbon with nitrogen in them.
...
**Predicts.** Enrichment builds the refractory reservoir even as it speeds the easy fraction - so nutrient loading and sediment legacy are not two independent problems but one, with a sign nobody has checked in the sea.

**Discriminated by.** Decay rate of refractory versus labile marine organic matter along a nutrient gradient.
...
**Discriminated by.** Sulphide and methane production rates against salinity along the Danish gradient, which spans the transition.
...
**Predicts.** Marine sediments release far more phosphate under anoxia than freshwater ones, so the Baltic phosphorus feedback is strong *because of the salt*. Combined with the iron shuttle, each episode leaves less iron and less binding capacity than the last.
...
**Predicts.** Riverine organic carbon does not disperse - it is deposited in a band right at the coast, which places the oxygen demand exactly where the shallow, stratified water and the bathing beaches are. A load figure at the river mouth therefore understates the local concentration of its effect.

**Discriminated by.** Sediment organic carbon along a salinity transect from each river mouth.
...
**Predicts.** Accumulation with no change in loading rate, simply because what arrives has changed. Connects the novo-chemical argument to the decay argument: a substance with no degradation terminus is a decay relay with no final stage.

**Discriminated by.** Decay rate of local versus novel organic material in the same water.
...
**Needs.** Cross-transplant incubations. Not run.
...
**Predicts.** The most thoroughly degraded areas are graded against the standard their own degradation set. This is the shifting baseline made administrative, and it is not a metaphor - it is what a classification key does when it types states rather than histories.

**Discriminated by.** Habitat classification against disturbance history, not against present sediment. Any area classified as sand whose historical charts or fisheries records show structure is a positive case.
...
Denmark's chlorophyll target is computed by ensemble modelling of a reference situation, then scaled by an EU-agreed ratio of [0.6](SOURCES.md#F-03924c3609). Both halves are choices, and neither is a measurement.
...
**Predicts.** Apparent change concentrated at times when the network changed.

**Discriminated by.** Recompute every trend on the subset of stations present throughout.

**Needs.** Station start and end dates. **Not held**: the register's dates are visit dates, not lifespans. Presence has to come from the observation record: [`hypodrafts/I1.md`](hypodrafts/I1.md). [What the data holds →](HYPOTHESIS_DATA.md#I1)
...
**Predicts.** Step changes at method transitions, shared across all stations using the same method regardless of geography.

**Discriminated by.** Result against sampling gear and sonde, which the raw record names.

**Needs.** SondeNr per measurement — the probe *number* is the identifier, and a changeover is when a bias shift has cause. **Present, and mostly filled**; where it is filled is where [I2](HYPOTHESES.md "Changing analytical method") is testable. [What the data holds →](HYPOTHESIS_DATA.md#I2)
...
**Predicts.** Apparent severity scales with visit count.

**Discriminated by.** Indicator against sampling effort per station-season.

**Needs.** Date of every visit per station — in the raw record. But `Dato` is `YYYYMMDD`: **the CTD extract has no time of day, and no column for it.** Oxygen has a diel cycle and daylight at [55](SOURCES.md#F-36b30a135c)°N runs [7](SOURCES.md#F-2ad04809f0)–[17](SOURCES.md#F-174fa66b89) h, so the diel phase sampled shifts with season. Class 6 **for the CTD extract**. The water-chemistry extract carries a clock value per sample; if chemistry shares cruises with CTD, it bounds the sampling hour and this becomes class 2. [What the data holds →](HYPOTHESIS_DATA.md#I3)
...
**Predicts.** Status shifts at definition changes with no change in any measurement.

**Discriminated by.** Recompute historical status under each successive definition.
...
**Predicts.** Trends present in corrected but not original results.

**Discriminated by.** Recompute everything on OriginalResultat.
...
**Predicts.** Discontinuities at 2007 shared across all stations that changed hands.
...
**Needs.** Nothing new. **A worked case was found**: negative oxygen in one two-month window in the Øresund approaches, where the water was in fact oxygenated and the sign inverted - a lead towards a shared processing step. Flagged, not removed, in [`data/areas/flags.json`](data/areas/flags.json). [What the data holds →](HYPOTHESIS_DATA.md#I7)
...
1. Find, for every entry, a data source at **measurement resolution** — one row per observation with its time and its position — not an aggregate. Aggregates are where the discriminating information goes.

2. Build the area-period panel: every observable, per water body, per period, with the gaps left visible as gaps rather than interpolated away.
