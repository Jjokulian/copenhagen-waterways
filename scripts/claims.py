...
def main(argv):
    ...
    # Each claim, and each thing claims rest on, is published once as a record of the
    # claims store (docs/static-async-data), which the reader's claim panel loads a
    # shard at a time; the fixed wording and the diagrams are made in the page.
    # Everything that is text goes through the compiler once, under this page's name,
    # so its numbers are checked and linked as they always were.
    ...
    live_claims = {cid: c for cid, c in claims.items() if not c.get("retired")}
    for cid, c in live_claims.items():
        r = {"k": c["kind"], "r": list(c["rests_on"])}
        put(cid, "t", resolve(d, c["claim"], cache)[0])
        if c.get("stance") == "theirs":
            r["th"] = 1
            put(cid, "h", resolve(d, c["holder"], cache)[0])
        said = _said_on(cid)
        if said:
            r["p"] = sorted(said)
        if c.get("because"):
    ...
    for nid in sorted({x for c in live_claims.values() for x in c["rests_on"] if x in nodes}):
        n = nodes[nid]
        r = {"k": n.get("kind")}
        for key in ("source", "said", "file", "commit"):
            if n.get(key):
                r[key] = n[key]
        if n.get("source"):
            src = d["sources"][n["source"]]
            sources[n["source"]] = [src.get("title", n["source"]), src["url"]]
        put(nid, "l", resolve(d, n.get("label", nid), cache)[0]
            .replace("\\n", " · ").replace("\n", " · "))
        if n.get("detail"):
            put(nid, "d", resolve(d, n["detail"], cache)[0])
        nrecs[nid] = r
    ...
    w("")
    w("*Generated by `scripts/claims.py` from `data/manual/claims.json`. Every claim on "
      "the site carries a † - click it to see what the claim rests on: its reasons, "
      "each thing beneath it, and a diagram of the whole chain. The claims are "
      "published once each as data the page loads (`docs/static-async-data/claims`), "
      "not printed here; this page is the key to what the panel shows. Each claim "
      "records what it saw when it was last confirmed, and nothing is published if "
    ...
    w("| circle, red dotted | **untraced** | where the trail stops: what was searched. "
      "Nothing below it is claimed |")
    w("")
    w("A claim is either this project's own or somebody else's. Ours says why it "
      ...
      "with their document pinned and the reasons they give.")
    w("")
    w("## Which pages make claims")
    w("")
    w(", ".join(f"[{p}]({p})" for p in sorted({c.get('page', '—') for c in live_claims.values()})))
    w("")
    ...
    body = "\n".join(o).rstrip("\n") + "\n\n" + SPLIT + "\n\n" + "\n\n".join(frag) + "\n"
    try:
        compiled = live.compile_doc(OUT, body)
    except live.Unjustified as e:
        log(str(e))
        return 1
    if "\\n" in compiled:
        log("  CLAIMS: a literal backslash-n in the output - a double-escaped newline")
        return 1
    head, tail = compiled.split(SPLIT, 1)
    parts = re.split(r"(?m)^§§ `([^`]+)` `([^`]+)` §§\n", tail)
    ...
    if ok:
        log(f"  published {len(recs):,} claims and {len(nrecs):,} things they rest on")
