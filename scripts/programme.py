...
def livestock_section():
    ...
    a("Two things follow from counting the animals, and the second one is the "
      "structural point of this whole document arriving from an unexpected "
      "direction.\n")
    a("**First: nitrogen is one constituent of a mixture, and it is the only one the "
      "ledger prices.** What leaves an animal is not nitrogen, it is faeces — "
      "phosphorus and potassium, organic carbon that is oxygen demand by another "
      "name, ammonium that consumes oxygen as a reductant rather than feeding "
      ...
      "register at quarterly resolution with no measurement of what became of them "
      "— which this project has already recorded as one of its open gaps.\n")
    a("**Second, and this is the part that should be uncomfortable for the argument "
      "in section 2: slurry storage is a basin.** Danish rules require months of "
      "storage capacity and forbid spreading through the autumn and winter, so the "
      "material accumulates in a covered tank, goes anoxic exactly as a sewer basin "
      "does, and is then emptied onto fields in a short legal window in spring. "
      "**Stored, reduced, concentrated, and released on a schedule.** The only "
      "difference from the flush this document spends section 2 attacking is what "
      "opens the valve: rainfall there, a calendar and a regulation here.\n")
    a("And the same two consequences follow. The delivery is a **pulse**, so an "
      "annual average cannot see it — the same objection this page makes to the "
      "overflow accounting, applied to the larger of the two sources. And the "
      "material that arrives is **reduced**, so its first act in a receiving water "
      "is to take oxygen rather than to feed anything, which is a different "
      "mechanism from the fertilisation story the whole framework is built on. What "
      "makes it worse rather than better than the sewer case is the timing: the "
      "spreading window opens in spring, upstream of the growing season, and the "
      "transport event is the first heavy rain onto a field that has just received "
      "it.\n")
    a("*What this does not establish.* No slurry chemistry has been measured by this "
      "project, no spreading-to-stream transport event has been observed in it, and "
      "the seasonal argument is a mechanism rather than a finding — the same status "
      "as the basin flush before anybody instruments one. What it does establish is "
    ...
    a("**So it is an open problem, and it has a design.** [`X23` in "
      ...
      "nutrients — and using faecal sterols and host-specific markers, which is "
      "what turns a concentration into an attribution. **The spreading window is "
      "the manipulation**; it happens every spring and is applied to the whole "
      "country at once, so the experiment is mostly a matter of being there with a "
...
def main():
    ...
    lean_ha = amager_ha * 0.01
    mm_yr = sed["m3_per_impervious_ha_per_year"] * amager_ha / (lean_ha * 1e4) * 1000
    a("The disposal argument in the next section is about a quantity nobody had "
      "sized, so here it is. Rain carries suspended solids at something like "
      f"**{sed['tss_mg_per_l_stated']:.0f} mg/l** off paved surface — a stated "
      "convention, in the middle of the range usually quoted — and a pond keeps at "
      f"least **{sed['capture_fraction_stated']*100:.0f}%** of them, which is the "
      "low end of the measured range in the table above. On Amager's "
      f"{amager_ha:,.0f} impervious hectares that is **{sed['solids_caught_t_per_year']:,.0f} "
      f"tonnes a year**, or about {sed['solids_caught_m3_per_year']:,.0f} m³ settled "
      f"in place; across the whole combined city, **{city_t:,.0f} tonnes** and "
      f"{city_m3:,.0f} m³.\n")
    a("**Which sets the cadence, and it is slower than the word *dredging* "
      f"suggests.** Spread over the lean {lean_ha:,.0f} ha sizing from the table "
      f"above, that is **about {mm_yr:.0f} mm a year** of accumulation. A cell "
      "reaches a hundred millimetres in roughly two decades. So the rotation is not "
      "a permanent industrial operation in a nature park — it is a cell out of "
      "service for a season, on a cycle measured in years, with the material "
      "leaving on ordinary lorries. That is the difference between this and the "
      "sewage works the phrase *light treatment* keeps summoning.\n")
    a("**And it is the local end of a much larger question.** Where those tonnes go "
      "is decided by assay rather than by category — deep-prior metals below a "
      "threshold become soil, cadmium and mercury do not, and a novo-chemical has "
      "to be destroyed rather than buried, which is the taxonomy in the next "
      "section. Denmark has no facility whose fluorine balance has been shown to "
      "close, and building one is a decade of permitting and construction, which is "
      "why [the industry question](#is-this-a-cost-or-is-it-an-industry) is not a "
      "coda to this document but a scheduling constraint on it. This is the "
      "quantity that would make it concrete: a few thousand tonnes a year, from one "
      "city, of material whose destination nobody has yet had to decide.\n")
    ...
    for t, s_ in [
        ...
        ("Product bans through REACH", "2–3 years, already started — and see below, "
                                       "because that row is at the wrong level"),
    ...
    a("A restriction under REACH is a decision about **market access**, taken once, "
      "for four hundred and fifty million people. It is the right instrument for "
      "the thing it does and it is the only row in that table nobody local can "
      "start. Which is worth saying plainly, because the rest of this document "
      "argues that acting in different places in different ways is how anything "
      "gets learned, and here is an instrument built to make that impossible on "
      "purpose: **a single market is a market where the same products are "
      "available everywhere, which is the definition of no contrast.**\n")
    a("But *banning a product from the market* and *refusing to buy or permit it* "
      "are different acts, and only the first is harmonised. What is left to the "
      "lower levels is not nothing:\n")
    a("| Level | Cannot | Can |")
    a("|---|---|---|")
    for lvl, cannot, can in [
        ("A household", "affect what is sold",
         "not buy it — which is the whole of the greywater argument in section 1: "
        ...
        ("A city", "ban a product from its shops",
         ...
         "attach conditions to leases, permits and contracts. Copenhagen already "
         "does a version of this on its own ground"),
        ("A region or a utility", "override an EU authorisation",
         "make use conditions in discharge permits and abstraction-zone rules — "
         "Denmark's own restricted-spraying zones around wellfields are exactly "
         "this instrument"),
        ("A member state", "unilaterally close its market",
         "tax, restrict a *use*, and apply for a derogation or safeguard. Denmark "
         "has done the first two — pesticide taxation, and the pre-harvest "
         "glyphosate restriction cited in section 5"),
    ]:
        a(f"| **{lvl}** | {cannot} | {can} |")
    a("")
    a("**So the achievable local instrument is a use and purchase restriction, not "
    ...
    a("The brittleness is worth naming because it is systemic rather than "
      ...
      "compound: the safeguard route exists but demands evidence of harm to a "
      "standard that is hard to reach precisely because nobody was allowed to vary "
      "the treatment. That circle — *no derogation without evidence, no evidence "
      "without a derogation* — is the same shape as the nutrient argument on this "
      "page, where the contest was never run and the winner was announced anyway. "
      "It is an argument for keeping the use and procurement levers deliberately "
      "open at every level below the market, not for leaving the EU's, which does "
      "the one thing no city can.\n")
    a("*What this does not claim.* This project has not read the case law, and the "
      "boundary between a lawful national use restriction and an unlawful barrier "
      "to trade is exactly where the arguing happens. The point stands at the "
