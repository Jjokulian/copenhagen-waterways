...
def render_sources(idx=None):
    ...
    for i, e in calcs:
        ...
        if t == "agg":
            ...
            if t == "nary":
                ...
                expr = f"[{sym}](#{sym}) applied to {ops[0]}"
            elif sym in ("neg", "abs"):
                expr = f"{sym}({ops[0]})"
            elif sym == "round":
    ...
    for i, e in sorted(quotes, key=lambda x: x[1]["src"][1]):
        ...
        if len(e["src"]) > 4 and e["src"][4]:
            w.append(f"- **Located by:** `{e['src'][4]}` - the value is read out of that "
                     "text when the page is built, never typed")
        used(i)
    if stateds:
